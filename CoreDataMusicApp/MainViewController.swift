...
extension MainViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.titleLabel.text = self.viewModel.albumTitle(index: indexPath.row)
        cell.artistLabel.text = self.viewModel.artistName(index: indexPath.row)
        ...
        cell.buttonPressedAction = { [] in
            ...
            if (self.viewModel.buttonStatus(index: indexPath.row) == 1 && hasChanged == 0) {
                cell.favoriteButton.setImage(UIImage(systemName: "star"), for: [])
                self.viewModel.changeButtonStatus(index: indexPath.row)
